...
export default async function EmployeeResourcesPage() {
  const employeeEmail = getPortalAuthenticatedEmailFromHeaders(await headers());
  const employeeRole = getPortalStaffRole(employeeEmail);

  if (!employeeEmail || !canAccessPortalAdmin(employeeRole)) {
