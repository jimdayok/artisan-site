...
function normalizeColorBrand(material, color, rawColor) {
  ...
  return materialBrand || colorBrand || cleanDisplayValue(rawColor) || "Unspecified";
}
...
export async function parsePriceList(config) {
  ...
  for (const rawRow of rawRows.slice(1)) {
    ...
    const brand = cleanDisplayValue(product?.brand || "Unmapped");
    const normalizedMaterial = cleanDisplayValue(material?.material || materialRaw);
    const materialColor = cleanDisplayValue(material?.materialColor || color?.color || colorRaw || "Unspecified");
    ...
    duplicatePriceConflicts.push(
      `${candidate.designType} / ${candidate.designStyle} / ${candidate.brand} / ${candidate.material} / ${candidate.materialColor} / ${candidate.colorBrand}: kept ${existing.edgedPrice}/${existing.uncutPrice}, saw ${candidate.edgedPrice}/${candidate.uncutPrice} from ${productNameRaw} ${materialRaw} ${colorRaw}`
    );

    if (rowCompleteness(candidate) > rowCompleteness(existing)) {
      normalizedRows.set(key, mergeRow(candidate, existing));
    } else {
      normalizedRows.set(key, mergeRow(existing, candidate));
    }
  }
