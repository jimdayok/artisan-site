...
function parseXmlText(xml, fileName, key) {
  let parsed;
...
export async function getXmlPriceFile(fileName) {
  ...
  try {
    response = await getR2S3Client(config).send(
    ...
    throw new Error(
      `Unable to fetch R2 object ${config.bucketName}/${key}: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  const xml = await bodyToText(response.Body);
  const parsed = parseXmlText(xml, fileName, key);
  ...
  return {
    ...
    fileSizeBytes: typeof response.ContentLength === "number" ? response.ContentLength : Buffer.byteLength(xml),
