...
function normalizeDesignStyle({ brand, designStyle, rawProductName }) {
  const cleanedStyle = stripSdPrefix(
    cleanDisplayValue(designStyle || rawProductName || "")
  );
...
export async function parsePriceList(config) {
  ...
  const productRows = Array.isArray(config.productLookupRows)
    ? config.productLookupRows
    : rowsToObjects(await readXlsxRows(config.productLookupPath));
  const materialRows = Array.isArray(config.materialLookupRows)
    ? config.materialLookupRows
    : rowsToObjects(await readXlsxRows(config.materialLookupPath));
  ...
  return {
    ...
    report: {
      sourceFiles: [
        config.rawPath,
        config.productLookupSourcePath || config.productLookupPath,
        config.materialLookupSourcePath || config.materialLookupPath,
