...
async function readXmlFile(rawDir, fileName) {
  const fullPath = path.join(rawDir, fileName);
  let xml;
  try {
    xml = await readFile(fullPath, "utf8");
  } catch {
    throw new Error(`Missing required raw XML file: ${fullPath}`);
  }
  ...
  try {
    ...
    throw new Error(`Malformed XML in ${fullPath}: ${error instanceof Error ? error.message : String(error)}`);
  }

  const st = await stat(fullPath);
  const rootName = Object.keys(parsed || {}).find((key) => !key.startsWith("?"));
  if (!rootName) {
    throw new Error(`Unable to find XML root in ${fullPath}`);
  }
...
export async function inspectDviRawPriceFiles(rawDir) {
  const xml = {};
  for (const fileName of DVI_REQUIRED_FILES) {
    xml[fileName] = await readXmlFile(rawDir, fileName);
  }
...
export async function generateDviPricingArtifacts(rawDir) {
