...
const PRODUCT_LOOKUP_CANDIDATES = [
  ...
  path.join("private-source", "portal", "lookup_docs", "Lookup.xlsx"),
];
...
async function readWorkbook(filePath, label) {
  ...
  return workbook.worksheets.map((worksheet) => {
    ...
    worksheet.eachRow((row) => {
      ...
      rows.push(values.map((value) => normalizeText(value)));
    });
    ...
    return {
      ...
      headerMap,
      rows,
      dataRows: rows.slice(1).filter((row) => row.some((cell) => normalizeText(cell))),
    };
  });
}
...
function productSheetsFromWorkbook(sheets, workbookLabel) {
  ...
  for (const candidate of candidates) {
    for (const row of candidate.sheet.dataRows) {
      ...
      if (keyToSource.has(key)) duplicateKeys.add(lenstyle);
      keyToSource.set(key, `${candidate.sheet.name}:${lenstyle}`);
      rows.push(entry);
    }
  }
  ...
  if (duplicateKeys.size > 0) {
    throw new Error(
      `${workbookLabel} contains duplicate lenstyle keys: ${[...duplicateKeys]
        .sort()
        .join(", ")}`
    );
  }
...
function arRowsFromWorkbook(sheets, workbookLabel) {
  ...
  let matchedSheetCount = 0;
  for (const sheet of sheets) {
    const dviIndex = findColumnIndex(sheet, ["dvi", "column a"]);
    const nameIndex = findColumnIndex(sheet, ["name", "column b"]);
    const brandIndex = findColumnIndex(sheet, ["brand", "column c"]);
