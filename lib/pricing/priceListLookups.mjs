...
export function stripSdPrefix(value) {
  return toText(value).replace(/^SD\s+/i, "").trim();
}
...
export function buildProductLookup(rows) {
  ...
  for (const row of rows) {
    ...
    if (!rawName) continue;
    const value = {
      designStyle: stripSdPrefix(toText(row.styleconsollidated ?? row[1])) || rawName,
...
export function colorBrandFromDescription(code, description) {
  ...
  if (normalizedDescription.includes("PHOTO")) return "Polarized Colors";
  if (normalizedDescription.includes("POLAR")) return "Polarized Colors";
  if (normalizedDescription.includes("MIRROR")) return "Polarized Colors";

  return "Polarized Colors";
}
