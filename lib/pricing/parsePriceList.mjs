...
export async function parsePriceList(config) {
  ...
  const productFallbacks = [
    {
      aliases: ["TOKAI AS SV"],
      value: { designStyle: "Tokai AS SV", designType: "SV", brand: "Tokai" },
    },
    {
      aliases: ["TOKAI EASYONE", "TOKAI EASYONE *", "EASYONE"],
      value: { designStyle: "Tokai EasyOne", designType: "Progressive", brand: "Tokai" },
    },
  ];
  for (const fallback of productFallbacks) {
    for (const alias of fallback.aliases) {
      const key = normalizeLookupKey(alias);
      if (!productLookup.has(key)) productLookup.set(key, fallback.value);
    }
  }
