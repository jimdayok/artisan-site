...
| `generate_lead` | Embedded Typeform `onSubmit`, or a signed Typeform webhook after a successful external form submission | `lead_type`, `form_name`, `traffic_context`, `site_version`, `lab_name` | Primary/key event | Implemented and deployed for both approved Typeforms |
...
Pipedrive now has all nine Lead/Deal text fields: `utm_source`, `utm_medium`, `utm_campaign`, `utm_content`, `utm_term`, `landing_page`, `referrer`, `site_version`, and `lab_name`. Reloaded UI verification on August 23, 2026 showed 96/100 custom fields in use. Both approved Typeforms originally used Pipedrive Classic. That legacy connection creates another person and organization on repeated submissions instead of reusing an exact email match, and its only management action is **Delete integration**.

The signed Typeform webhook includes a server-only Pipedrive attribution step. In `classic` mode it finds the person created by Pipedrive Classic and adds attribution to a matching lead/deal or creates one neutral Leads Inbox record. In `upsert` mode it replaces the legacy connector: it reuses an exact-email person, preferring the established person when historical duplicates exist, or creates one organization and person only when no exact match exists. It then updates or creates the neutral Lead. It never creates a pipeline deal. New leads use the Typeform response token as a stable origin identifier so webhook retries do not intentionally create duplicate Leads.

Only the approved business contact fields are sent to Pipedrive: primary name, business email, business phone, and company. Free-text messages, Social Security numbers, bank references, trade references, and all other application answers remain in Typeform. None of the business contact fields or form answers are sent to GA4. The landing page is stored without its query string, and the referrer is stored as origin only. If Pipedrive is unavailable or not configured, the Typeform response and GA4 lead event continue independently.
